...
class TestOracleWMSTargetCli:
    ...
    async def test_execute_with_config_file(self) -> None:
        ...
        with patch.object(cli, "_load_config", return_value=config_data), \
             patch("flext_target_oracle_wms.cli.SingerTargetOracleWMS") as mock_target_class, \
             patch("sys.stdin", []):  # Empty stdin
    ...
    async def test_execute_with_singer_messages(self) -> None:
        ...
        with patch("flext_target_oracle_wms.cli.SingerTargetOracleWMS") as mock_target_class, \
            ...
            mock_target = MagicMock()
            mock_target.setup = AsyncMock(return_value=FlextResult.ok(None))
            mock_target.process_schema_message = AsyncMock(return_value=FlextResult.ok(None))
            mock_target.process_record_message = AsyncMock(return_value=FlextResult.ok(None))
    ...
    async def test_execute_exception_handling(self) -> None:
        ...
        with patch("flext_target_oracle_wms.cli.SingerTargetOracleWMS", side_effect=Exception("Test error")):
            result = await cli.execute()
            assert not result.is_success
...
class TestMainFunction:
    ...
    def test_main_basic_execution(self) -> None:
        ...
        with patch("flext_target_oracle_wms.cli.OracleWMSTargetCli") as mock_cli_class, \
