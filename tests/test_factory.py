...
class TestFlextTargetFactory:
    ...
    def test_create_target_unknown_preset(self, mock_target_class: MagicMock) -> None:
        ...
        with patch('flext_target_oracle_wms.factory.logger') as mock_logger:
            ...
            assert "Unknown preset: unknown_preset" in mock_logger.warning.call_args[0][0]

    @patch('flext_target_oracle_wms.factory.SingerTargetOracleWMS')
    def test_create_target_exception_handling(self, mock_target_class: MagicMock) -> None:
        ...
        assert result.error is not None and "Failed to create Oracle WMS target" in result.error
    ...
    def test_create_from_config_dict_missing_fields(self) -> None:
        ...
        assert not result.is_success
        assert result.error is not None and "Missing required configuration fields" in result.error
    ...
    def test_create_from_config_dict_exception(self) -> None:
        ...
        with patch.object(FlextTargetFactory, 'create_target', side_effect=RuntimeError("Config error")):
            result = FlextTargetFactory.create_from_config_dict(config)
            
            assert not result.is_success
            assert result.error is not None and "Failed to create target from config" in result.error
...
class TestFlextTargetMonitoringFactory:
    ...
    def test_monitoring_factory_initialization(self, mock_monitor_class: MagicMock) -> None:
    ...
    @patch('flext_target_oracle_wms.factory.FlextObservabilityMonitor')
    def test_create_monitored_target_base_failure(self, mock_monitor_class: MagicMock) -> None:
        ...
        with patch.object(factory.factory, 'create_target', return_value=FlextResult.fail("Base creation failed")):
            result = factory.create_monitored_target(
    ...
    def test_create_monitored_target_exception(
        ...
        with patch.object(factory.factory, 'create_target', side_effect=RuntimeError("Monitor error")):
            result = factory.create_monitored_target(
                base_url="https://error.wms.oracle.com",
                username="error_user",
                password="error_password"
            ...
            assert not result.is_success
            assert result.error is not None and "Failed to create monitored target" in result.error
...
class TestFactoryConvenienceFunctions:
    ...
    def test_create_monitored_oracle_wms_target(self, mock_factory_class: MagicMock) -> None:
...
class TestFactoryIntegration:
    ...
    def test_factory_solid_principles_compliance(self) -> None:
        ...
        from flext_target_oracle_wms.factory import create_oracle_wms_target
