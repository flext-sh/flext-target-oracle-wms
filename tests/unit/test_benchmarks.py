...
class TestTypeConverterBenchmarks:
    ...
    def test_convert_string_performance(self) -> None:
        ...
        start = time.time()
        for _ in range(c.TargetOracleWms.Tests.Benchmarks.PERF_ITERATIONS):
            converter.convert_singer_to_oracle("string", "hello")
        elapsed = time.time() - start
        assert elapsed < c.TargetOracleWms.Tests.Benchmarks.PERF_THRESHOLD_SEC
...
class TestTableManagerBenchmarks:
    ...
    def test_register_and_lookup_performance(self) -> None:
        ...
        start = time.time()
        for i in range(c.TargetOracleWms.Tests.Benchmarks.PERF_ITERATIONS):
            name = f"stream_{i}"
