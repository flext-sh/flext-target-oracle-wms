...
class TestTargetCreationRequest:
    ...
    def test_custom_environment(self) -> None:
        req = TargetCreationRequest(base_url="https://x", username="u", password="p", environment="production")
...
class TestMonitoredTargetCreationRequest:
    ...
    def test_default_monitor_name(self) -> None:
        req = MonitoredTargetCreationRequest(base_url="https://x", username="u", password="p")
...
class TestFlextTargetFactory:
    ...
    def test_presets_contain_expected_keys(self) -> None:
        assert set(FlextTargetFactory.PRESETS.keys()) == {"development", "production", "testing"}

    @patch(_PATCH_TARGET)
    def test_create_target_success(self, mock_cls: object) -> None:
        req = TargetCreationRequest(base_url="https://x", username="u", password="p")
    ...
    def test_create_target_with_preset(self, mock_cls: object) -> None:
        req = TargetCreationRequest(base_url="https://x", username="u", password="p", preset="testing")
        result = FlextTargetFactory.create_target(req)
        assert result.is_success

    @patch(_PATCH_TARGET)
    def test_create_target_with_additional_config(self, mock_cls: object) -> None:
    ...
    @patch(_PATCH_TARGET)
    def test_create_from_config_dict_success(self, mock_cls: object) -> None:
    ...
    def test_create_from_config_dict_missing_base_url(self) -> None:
        result = FlextTargetFactory.create_from_config_dict({"username": "u", "password": "p"})
        assert result.is_failure
        assert "base_url" in (result.error or "")

    def test_create_from_config_dict_missing_username(self) -> None:
        result = FlextTargetFactory.create_from_config_dict({"base_url": "https://x", "password": "p"})
        assert result.is_failure
        assert "username" in (result.error or "")

    def test_create_from_config_dict_missing_password(self) -> None:
        result = FlextTargetFactory.create_from_config_dict({"base_url": "https://x", "username": "u"})
        assert result.is_failure
