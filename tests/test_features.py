...
class LoadTestMetrics:
    ...
    def records_per_second(self) -> float:
        ...
        return (
            self.records_processed / self.duration_seconds
...
@pytest.fixture
def production_config() -> dict[str, t.GeneralValueType]:
...
def production_target(
    production_config: dict[str, t.GeneralValueType],
...
class TestProductionLoadTesting:
    ...
    def test_high_volume_record_processing(
        ...
        for batch_start in range(0, records_count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                batch_tasks.append(
                    production_target.process_record_message(record_message),
                )
            ...
            if (batch_start + batch_size) % 2000 == 0:
                ...
                assert current_memory - memory_start < 500, (
        ...
        assert metrics.records_per_second >= 100, (
            f"Performance too low: {metrics.records_per_second:.1f} rps"
        )
    ...
    def test_concurrent_stream_processing(
        ...
        assert metrics.records_per_second >= 200, (
            f"Concurrent performance too low: {metrics.records_per_second:.1f} rps"
        )
        assert metrics.success_rate >= 99.0, (
            f"Concurrent success rate too low: {metrics.success_rate:.1f}%"
        )
        assert total_processed >= expected_records * 0.99, (
    ...
    def test_error_recovery_and_resilience(
        ...
        test_scenarios: list[dict[str, t.GeneralValueType]] = [
        ...
        assert overall_success_rate >= 60, (
    ...
    def test_memory_efficiency_large_records(
        ...
        for i in range(records_count):
            ...
            large_record = {
                ...
                "large_object": {
                    f"key_{j}": f"value_{j}_{'x' * 10}" for j in range(100)
                },  # Large object
                "binary_data": "0"
                * (large_record_size_kb * 100),  # Simulated binary data
                "metadata": {
                    "batch": i // 100,
            ...
            if i % 50 == 0:
                ...
                expected_max_growth = (
                    (i + 1) * large_record_size_kb * 0.1 / 1024
                )  # Allow 10% overhead in MB
        ...
        memory_efficiency = (final_memory - initial_memory) / (
            records_count * large_record_size_kb / 1024
        )

        # Memory efficiency assertions
        assert memory_efficiency < 2.0, (
            f"Memory efficiency too low: {memory_efficiency:.2f}x overhead"
        )
        assert final_memory - initial_memory < 500, (
            f"Total memory usage too high: {final_memory - initial_memory:.1f}MB"
        )
...
class TestProductionDataIntegrity:
    ...
    def test_transactional_consistency(
        ...
        processed_orders: list[dict[str, t.GeneralValueType]] = []
        ...
        for i, processed_order in enumerate(processed_orders):
            ...
            assert (
                abs(float(processed_order["total_amount"]) - expected_total) < 0.01
            ), (
...
class TestProductionEdgeCases:
    ...
    def test_extreme_data_values(
        ...
        extreme_records = [
            # Large numbers
            {
                ...
                "big_number": 1.7976931348623157e308,  # Near max float
            ...
            # Boundary values
            {
                ...
                "big_number": -1.7976931348623157e308,  # Near min float
        ...
        for record_data in extreme_records:
            ...
            try:
                ...
                if result.success:
                    ...
                    warnings.append(
                        f"Record {record_data['id']} failed: {result.error}",
                    )

            except Exception as e:
                warnings.append(f"Record {record_data['id']} caused exception: {e!s}")

        # Most extreme values should be handled gracefully
        success_rate = (processed_count / len(extreme_records)) * 100
        assert success_rate >= 80, (
            f"Extreme value handling too low: {success_rate:.1f}%"
        )
    ...
    def test_malformed_messages(
        ...
        malformed_messages = [
            ...
            {
                "stream": "malformed_test",
                "record": {"id": "2", "data": "test"},
            },  # Missing type
            # Invalid field types
            {
                "type": 123,
                "stream": "malformed_test",
                "record": {"id": "3", "data": "test"},
            },  # Non-string type
            {
                "type": "RECORD",
                "stream": 456,
                "record": {"id": "4", "data": "test"},
            },  # Non-string stream
            {
                "type": "RECORD",
                "stream": "malformed_test",
        ...
        for i, malformed_message in enumerate(malformed_messages):
            try:
                ...
                handled_gracefully += 1
                errors_logged.append(f"Message {i} exception: {e!s}")

        # System should handle malformed messages gracefully
        graceful_rate = (handled_gracefully / len(malformed_messages)) * 100
        assert graceful_rate >= 80, (
            f"Malformed message handling too low: {graceful_rate:.1f}%"
        )
    ...
    def test_resource_exhaustion_scenarios(
        ...
        assert connection_success_rate > 50, (
            f"Connection handling too poor: {connection_success_rate:.1f}%"
        )
